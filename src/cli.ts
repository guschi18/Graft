...
program
  ...
  .action(async (query: string, dir: string, opts: { limit: string; source?: boolean; full?: boolean; in?: string; json?: boolean; refresh?: boolean; graphRank?: boolean }) => {
...
program
  ...
  .action(async (dir: string, opts: { extensions?: string[]; json?: boolean }) => {
    const checkGlobalDir = program.opts<GlobalOpts>().dir;
    if (readWorkspace(resolve(dir), checkGlobalDir)) {
      await runWorkspaceCheck(resolve(dir), checkGlobalDir);
      return;
    }
    const engine = engineFrom();
